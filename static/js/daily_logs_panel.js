/**
 * daily_logs_panel.js
 * Controls Column 4 merged panel: "How was your day?" (Reflection) & "Logs" (8:00 AM - 8:00 PM Timeline).
 */
document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatSlotRangeLabel(slotStr) {
        ...
        try {
            ...
            let endM = (m + 15) % 60;
            let endH = endM === 0 ? (h + 1) % 24 : h;
            return `${slotStr} - ${String(endH).padStart(2, '0')}:${String(endM).padStart(2, '0')}`;
        } catch (e) {
            return slotStr;
        }
    }

    // ── Switch Column 4 Tabs ──
    ...
    // ── Render Timeline Feed (Always 8:00 AM to 8:00 PM) ──
    function renderTimelineSlots(slotsMap = {}) {
        ...
        timelineFeed.innerHTML = ALL_8TO8_SLOTS.map(slot => {
            ...
            const rangeLabel = formatSlotRangeLabel(slot);
            ...
            return `
                <div class="timeline-slot-row ${isCurrent ? 'is-current-slot' : ''} ${isLogged ? 'is-logged' : ''}" data-slot="${slot}">
                    <span class="slot-time-badge" title="${rangeLabel}">${slot}</span>
                    <input 
                        type="text" 
                        class="slot-input" 
                        data-slot="${slot}" 
        ...
        if (logsCounter) {
            logsCounter.textContent = `${loggedCount}/${ALL_8TO8_SLOTS.length} logged`;
        }
        ...
        timelineFeed.querySelectorAll('.slot-input').forEach(input => {
            ...
            input.addEventListener('keydown', (e) => {
                if (e.key === 'Enter') {
                    e.preventDefault();
                    triggerSave();
                    // Auto-focus next slot input for keyboard speed
        ...
        timelineFeed.querySelectorAll('.slot-cat-select').forEach(sel => {
            sel.addEventListener('change', async (e) => {
                ...
                if (!text) {
                    // Update cache category preference
